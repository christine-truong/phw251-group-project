...
Sponsored by the State of California's Department of Public Health (CDPH), the data source for this project is the 2011 California Smokers Cohort (CSC), which is a part of the California Tobacco Surveys (CTS) that collected information on the prevalence of tobacco use and behaviors among smokers in California in order to inform tobacco use prevention efforts. To help CDPH better assess the effectiveness of smoking cessation strategies, the 2011 CSC data specifically investigates characters associated with quitting behavior among only smokers identified through telephone contacts purchased from data brokers, as well as through the California Health Interview Survey Longitudinal Smokers Survey (CLSS) between July 8, 2011 and December 8, 2011.
\newline
...
race_data <-read_csv("ca_csc_outcome_race_data.csv",
            col_select = c(NERVOUS, WORRYING, PROBINTR,
                           PROBDOWN, OPHYSILL, ASTHMA, HEARTDIS,
                           DIABETES, race01, race02, race03,
...
In the data set containing demographic/behavioral/race information about the study participants (i.e., data set labeled race_data), our variables of interest are NERVOUS, WORRYING, PROBINTR, PROBDOWN, OPHYSILL, ASTHMA, HEARTDIS, DIABETES,
...
\newline OPHYSILL" -- "Has a physician ever told you that you have any other physical illness?"
...
For variables associated with physical and mental illnesses (i.e., OPHYSILL, ASTHMA, HEARTDIS, DIABETES), the unique values are "No," and "Yes."
\newline

For variables associated with race (i.e., race01 through race15), the only unique value is "Yes."
\newline
