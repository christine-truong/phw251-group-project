...
```{r tables, Smoker Data wherebuy}

library(kableExtra)

#Smoker Data Table: wherebuy

table_smoker_wherebuy <- table(smoker_data_3$wherebuy)

kable(table_smoker_wherebuy, 
      booktabs=T, 
      col.names=c("Location", "Frequency"),  
      align='lcccc', 
      caption="Cigarette Buying Location",
      format.args=list(big.mark=","))%>%
  kable_styling(latex_options = "HOLD_position")
```

\newpage
```{r tables, Smoker Data howmany}
#Smoker Data Table: howmany

table_smoker_howmany <- smoker_data_3 %>% select(wherebuy,howmany) %>% group_by(wherebuy) %>%
  summarize(howmany=mean(howmany))

kable(table_smoker_howmany, 
      booktabs=T, 
      col.names=c("Location", "Average Cigarettes smoked"),  
      align='lcccc', 
      caption="Mean average of cigarettes smoked in the past 30 days based on buying location",
...
```{r tables, Race Data}
#Race Data Table: nervous and asthma

table_race <- table(race_data_2$nervous,race_data_2$asthma)

kable(table_race, 
      booktabs=T, 
      col.names=c("Yes", "No"),  
      align='lcccc', 
      caption="Feelings of Nervousness among Smokers with Asthma",
      format.args=list(big.mark=","))%>%
  kable_styling(latex_options = "HOLD_position") 
