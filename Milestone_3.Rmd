...
#up in the table we are creating. We then grouped by `othmenill` and calculated
#the number of smokers with previously diagnosed mental illness per nervous/anxious/
#on edge level-of-feeling category.


### WILL FIX THIS LATER###
nervous_othmenill_no_na <- race_data_2 %>%
  select(nervous, othmenill) %>%
  drop_na(nervous, othmenill) %>%
  pivot_wider(names_from = "othmenill", values_from = nervous)
...
kable(nervous_othmenill_no_na, 
      booktabs=T, 
      col.names=c("Nervousness/Anxiousness", "No Mental Illness"),  
      align='lcccc', 
      caption="Feelings of Nervousness/Anxiousness/On Edge Among Smokers with Mental Illness",
      format.args=list(big.mark=","))%>%
  kable_styling(latex_options = "HOLD_position") 
