...
##DELETE THESE THREE PARAGRAPHS LATER: **one print quality tables per scenario**
...
#table_data<-joined_smoking_df%>%filter(!is.na(pack_years))%>%group_by(asthma, diabetes, othmenill)%>%summarize(avg_pack_years=sum(pack_years)/n())

#aggregate(avg_pack_years~asthma+diabetes, table_data, mean)
