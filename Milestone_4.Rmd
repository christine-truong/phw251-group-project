...
table_data<-joined_smoking_df%>%filter(!is.na(pack_years))%>%select(pack_years, asthma, heartdis, diabetes, othmenill)%>%group_by(asthma, heartdis, diabetes, othmenill)%>%summarize(avg_pack_years=sum(pack_years)/n())%>%
kable(table_data)
 #select(pack_years, asthma, heartdis, diabetes, othmenill)%>%summarize(avg_pack_years=sum(pack_years)/n())
  
  
  
 # kbl(names_from=c(asthma, heartdis, diabetes, othmenill),values_from=pack_years) %>%
  #data.frame()
```
